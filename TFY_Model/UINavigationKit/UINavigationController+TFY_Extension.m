...
/** 自定义导航栏左侧返回按钮文本 */
- (void)tfy_customLeftbarbuttonTitle:(UINavigationItem *)navigation barButtonColorWithHexString:(NSString *)color titlesize:(CGFloat)fontSize barbuttonTitle:(NSString *)barButtonTitle target:(id)target selector:(SEL)selector complete:(void(^)(UIButton *itemButton))complete{
...
/** 自定义导航栏右侧返回按钮文本 */
- (void)tfy_customRightbarbuttonTitle:(UINavigationItem *)navigation barButtonColorWithHexString:(NSString *)color titlesize:(CGFloat)fontSize barbuttonTitle:(NSString *)barButtonTitle target:(id)target selector:(SEL)selector complete:(void(^)(UIButton *itemButton))complete{
...
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item {
    ...
    if([vc respondsToSelector:@selector(tfy_navigationBarWillReturn)]) {
        shouldPop = [vc tfy_navigationBarWillReturn];
    }
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
    if (gestureRecognizer == self.interactivePopGestureRecognizer) {
        UIViewController *vc = [self topViewController];
        if([vc respondsToSelector:@selector(tfy_navigationBarWillReturn)]) {
            return [vc tfy_navigationBarWillReturn];
        }
